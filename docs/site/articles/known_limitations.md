...
- **Anonymous types**: their properties are read-only, so only get-only property shapes can be satisfied. A property whose type contains an anonymous type as a type argument or array element (e.g. `new[] { new { A = 1 } }`) can't be named by the generated adapter ([`IFITQUACKS001`](diagnostics.md#ifitquacks001)).
- **Identity**: adapters forward `Equals`, `GetHashCode` and `ToString`, but `==` and `ReferenceEquals` on the shape compare the boxed adapters, which differ for every conversion. Use `Equals` or `Duck.Unwrap`.
