...
Each fallback overload is marked `[OverloadResolutionPriority(-1)]`, so it never takes a call from an overload you wrote. With `Greet(INamed)` and `Greet(object)`, `Greet(42)` binds to `Greet(object)`. `Greet(new Person())` binds there too, and is then intercepted and redirected to `Greet(INamed)` if C# would pick that overload had `Person` implemented `INamed`. An exact `Greet(Person)` still wins. The attribute needs C# 13 (it is generated on .NET 8), and without it a `[DuckTyped]` method with such an overload is reported with [`IFITQUACKS004`](diagnostics.md#ifitquacks004).
...
`Duck.Merge` holds one field per value and takes every member from the first field whose type implements the member's interface, else from the first field providing it structurally. `IDuckAdapter.Value` and `ToString` forward to the first value; `Equals` and `GetHashCode` cover every value, so two merges are equal only when all their values are.
